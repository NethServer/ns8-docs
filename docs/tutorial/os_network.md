...
3.  The network default gateway IP address. It is an IP address in the same network of your node, for instance `192.168.12.1`
4.  The DNS server address. It can be a public DNS service, like Google `8.8.8.8` or `8.8.4.4`, Cloudflare `1.1.1.1`, or a private DNS server. In small environments it could be the gateway host itself.
