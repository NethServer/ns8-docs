...
Log records generated by any cluster node are collected and stored in the leader node. See [Logs persistence](#logs-persistence-section) for additional information.
...
By default, log searches are directed to the active Loki instance. If there are [inactive Loki instances](#inactive-loki-section) within the cluster, it is possible to select them to search past log entries.
...
## Logs persistence {#logs-persistence-section}
...
### Inactive Loki instances {#inactive-loki-section}
...
To enable forwarders, navigate to `Settings` -\> `System Logs` page, where all Loki instances, including the active one, are listed. Here, you can configure services by opening the Loki card menu.
...
- **Export start date**: Specify from when logs should be forwarded. You can choose the last timestamp[^4] or manually enter a date and time.
...
Once the form is saved, the export procedure begins. After a few minutes, the cluster security logs will be available at <https://naradmin.nethesis.it/>.

Only security logs are forwarded to the Cloud Log Manager. To forward the full log stream, which may generate considerable data, refer to [Syslog](#syslog-section).
...
[^3]: Last timestamp indicates the last time that the forwarder successfully sent logs. This allows the forwarder to resume forwarding from where it was interrupted, ensuring continuity in log management.

[^4]: Last timestamp indicates the last time that the forwarder successfully sent logs. This allows the forwarder to resume forwarding from where it was interrupted, ensuring continuity in log management.
