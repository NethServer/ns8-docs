...
- [Rclone-compatible provider](#rclone-provider), for other widely supported cloud storages
...
2.  Create a Podman volume named `backup00`:
    ...
    :::warning
    Mount the disk by its `by-id` path. If the disk fails or I/O errors
    occur, the mount fails instead of writing data to the root
    filesystem. Avoid bind mounts.
    :::
...
You can set up any Rclone-compatible provider as a backup destination by
using a short INI configuration[^wini]. An exhaustive list of [supported
cloud storage providers][rco] is maintained on the official Rclone web
site.
...
In some cases, cloud providers document working Rclone configuration
examples in their manuals or allow you to download a customized
configuration from their management web site. Before writing the
configuration yourself, check your provider documentation or public
forums.

This is an example configuration for a *Unix SFTP server[^sftp]* backup destination with password authentication:
...
As required by Rclone, the `pass` field must contain an *obscured*
password string. You can generate it on NS8 with this command:

```sh
echo 'MySuperSecret' | podman exec -i rclone-gateway rclone obscure -
```

To convert an obscured password back to clear text, run:
...
Use the optional `Path` field to choose where the two-level directory
structure generated by the backup procedure is stored. Use `/` (slash)
as the directory separator. If `Path` is not set, the root of the
remote destination is used. This may be undesirable if the destination
is shared by many NS8 clusters.
