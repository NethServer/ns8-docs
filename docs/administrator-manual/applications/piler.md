...
[Piler](https://www.mailpiler.org/) is an open-source mail archiving solution.

This Piler application for NS8 configures a [Mail server](mail.md) instance to act as its archive, recording any message sent or received by the server for a configurable duration. If a message is lost, authorized users (known as "auditors") can access the Piler UI, search for the message in the archive, and send it back to the original recipient.
...
2.  After the initial configuration, Piler starts with default credentials. For security reasons, refer to section [Default credentials](#default-piler-credentials) to change the default passwords immediately.
...
- [Administration tasks](https://docs.mailpiler.com/piler-ee/administering/#administrator-tasks), to manage users, permissions, and check the audit log.
- [Rules and policies](https://docs.mailpiler.com/piler-ee/administering/#rules-and-policies), to define what messages are archived and how long they will stay. For example, you can define a special policy for spam messages.
...
1.  Email messages received with [Imapsync](imapsync.md) are not archived.
